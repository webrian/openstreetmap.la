...
L.Control.Status = L.Control.Attribution.extend({
    ...
    setText: function(text){
        this.options.tpl.overwrite(this.getContainer(),[text]);
    }
});
...
Ext.onReady(function(){
    ...
    var editTmpText = "<a class=\"external\" href=\"http://www.openstreetmap.org/edit?lat={lat}&lon={lon}&zoom={zoom}\">edit the map</a>";
    ...
    var attribution = 'CC-By-SA <a href=\"http://www.osm-tools.org\">www.osm-tools.org</a>, <a href=\"http://www.openstreetmap.org\">OpenStreetMap</a>';
    ...
    map.addLayer(layers[currentLang]);
    ...
    // Create the necessary action buttons for the toolbar
    var toolbarItems = [
    searchComboBox,
    '->',
    {
        ...
        enableToggle: true,
        handler: function(evt) {
            map.removeLayer(layers['en']);
            map.addLayer(layers['lo']);
            currentLang = 'lo';
        },
        listeners: {
            mouseover: function(button, evt){
                statusControl.setText('\u0e9e\u0eb2\u0eaa\u0eb2\u0ea5\u0eb2\u0ea7');
            },
            mouseout: function(button, evt){
                statusControl.setText('');
            }
        ...
        enableToggle: true,
        handler: function(evt) {
            map.removeLayer(layers['lo']);
            map.addLayer(layers['en']);
            currentLang = 'en';
        },
        listeners: {
            mouseover: function(button, evt){
                statusControl.setText('English');
            },
            mouseout: function(button, evt){
                statusControl.setText('');
            }
    ...
    var startCombo = new Ext.ux.LocationComboBox({
        emptyText: 'Search village, point of interest, etc.',
        listeners: {
            'select': function(combo, record, index){
                // Get the coordinates from the selected record
                var c = Fgh.decode(record.data.hash);
    ...
    var routeSummaryTpl = dh.createTemplate({
        tag: 'div',
        html: 'Distance: <b>{0} km</b><br>Duration: <b>{1} h {2} mins</b>'
    });
        ...
        if(startmarker && endmarker) {
            ...
            Ext.Ajax.request({
                ...
                success: function(response){
                    ...
                    var latlngs = []
                    Ext.each(responseObj.features[0].geometry.coordinates,function(item, index, allItems){
                        latlngs.push(new L.LatLng(item[1],item[0]));
                    });
                    ...
                    route.on('mouseover', function(e){
                        statusControl.setText('Click to add new via point');
                    });
                    ...
                    var instructions = responseObj.features[0].properties.route_instructions
                    ...
                    for (var i = 0; i < instructions.length; i++){
                        ...
                        var step_distance = instructions[i][5];
                        ...
                        routeInstructionsTpl.append(instructionsDiv, [(i+1), cls, instructions[i][0], street, dist]);
                    }

                    // Calculate hours and minutes from the total time in seconds
                    var total_time = responseObj.features[0].properties.total_time;
...
Ext.ux.LocationComboBox = Ext.extend(Ext.form.ComboBox, {
    ...
    initComponent: function(){
        ...
        var config = {
            emptyText: 'Search village, hotel, restaurant, shop, etc.',
